...
// Define the admin panel password
define('ADMIN_PANEL_PASSWORD', 'admin123');

// Check if the admin has entered the correct panel password
if (!isset($_SESSION['admin_panel_authenticated'])) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['panel_password'])) {
        if ($_POST['panel_password'] === ADMIN_PANEL_PASSWORD) {
            $_SESSION['admin_panel_authenticated'] = true;
        } else {
            $auth_error = "Incorrect admin panel password.";
        }
    }
}
...
if (!isset($_SESSION['admin_panel_authenticated'])) {
    ...
    <body>
        <div class="container mt-5">
            ...
            <?php if (isset($auth_error)) echo "<p class='text-danger'>$auth_error</p>"; ?>
            <form method="POST">
                <div class="mb-3">
...
if (isset($_POST['approve_document'])) {
    ...
    $stmt->execute([$_POST['document_path'], $_POST['achievement_id']]);
}
...
if (isset($_POST['send_notification'])) {
    ...
    $stmt->execute([$_POST['notification_message'], $_POST['recipient_role'], $_SESSION['user_id']]);
}
                        ...
                        <td>
                            <?php if ($user['role'] === 'student'): ?>
                                <form method="POST" style="display:inline;">
                                    ...
                                    <button type="submit" name="approve_profile" class="btn btn-success btn-sm">Approve</button>
                                    <button type="submit" name="reject_profile" class="btn btn-danger btn-sm">Reject</button>
                                </form>
                            <?php endif; ?>
                            <form method="POST" style="display:inline;">
                                <input type="hidden" name="user_id" value="<?php echo $user['id']; ?>">
